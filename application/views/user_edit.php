                ...
                <form class="required-form" autocomplete="off" action="<?php echo site_url('admin/users/edit/' . $user_id); ?>" enctype="multipart/form-data" method="post">
                    <div id="progressbarwizard">
                        <ul class="nav nav-pills nav-justified form-wizard-header mb-3">
                            ...
                            <li class="nav-item">
                                <a href="#course_enrol" data-toggle="tab" class="nav-link rounded-0 pt-2 pb-2">
                                    <i class="dripicons-archive mr-1"></i>
                                    <span class="d-none d-sm-inline"><?php echo get_phrase('course'); ?></span>
                                </a>
                            </li>
                            <li class="nav-item">
                        ...
                        <div class="tab-content b-0 mb-0">
                                ...
                                <div class="row">
                                    <div class="col-12">
                                        ...
                                        <div class="form-group row mb-3">
                                            ...
                                            <div class="col-md-9">
                                                <input type="tel" class="form-control" id="last_name" name="last_name" value="<?php echo $user_data['last_name']; ?>" required>
                                            </div>
                                        </div>
                                        <div class="form-group row mb-3">
                                            <label class="col-md-3 col-form-label" for="celular"><?php echo get_phrase('cellphone'); ?> </label>
                                            <div class="col-md-9">
                                                <input type="text" class="form-control phone-ddd-mask" placeholder="Ex.: (00) 0000-0000" id="cellphone" name="celular" value="<?php echo $user_data['celular']; ?>">


                                            </div>
                                        </div>
                                        <div class="form-group row mb-3">
                                            <label class="col-md-3 col-form-label" for="telefone"><?php echo get_phrase('phone'); ?></label>
                                            <div class="col-md-9">
                                                <input type="text" class="form-control" id="phone" name="telefone" value="<?php echo $user_data['telefone']; ?>" >
                                            </div>
                                        </div>
                                        <div class="form-group row mb-3">
                                            <label class="col-md-3 col-form-label" for="cpf"><?php echo get_phrase('cpf'); ?></label>
                                            <div class="col-md-9">
                                                <input type="text" class="form-control" id="cpf" name="cpf" value="<?php echo $user_data['cpf']; ?>" >
                                            </div>
                                        </div>
                            ...
                            <div class="tab-pane" id="course_enrol">
                                <div class="row">
                                    <div class="col-12">
                                        ...
                                        <div class="table-responsive-sm mt-4">
                                            <table id="dataTableStudent" name="dataTableStudent" class="table table-striped table-centered mb-0">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th><?php echo get_phrase('enrolment_date'); ?></th>
                                                         <th><?php echo get_phrase('days_expiretion'); ?></th>
                                                ...
                                                <tbody>
                                                    ...
                                                    <?php $enrolled_courses = $this->crud_model->enrol_history_by_user_id($user_data['id']); ?>
                                                    ...
                                                    <?php foreach ($enrolled_courses->result_array() as $enrolled_course) : ?>
                                                        ...
                                                        <tr>
                                                            ...
                                                            ?>
                                                            <td><?= $course_details['id'] ?></td>

                                                            <td><strong><a href="<?php echo site_url('admin/course_form/course_edit/' . $course_details['id']); ?>" target="_blank"><?php echo $course_details['title']; ?></a></strong></td>
                                                            <td><input class="form-control" id="dateCourse<?= $enrolled_course['id'] ?>" name="dateCourse" type="date" value="<?php echo date('Y-m-d', $enrolled_course['date_added']); ?>"></td>
                                                            <td><input class="form-control" id="days_experetion<?= $enrolled_course['id'] ?>" name="days_expiretion" type="number" value="<?php echo $enrolled_course['days_expiretion']; ?>"></td>
                                                            <td>
                                                                <div class="custom-control custom-switch">

                                                                    <input class="custom-control-input"  type="checkbox" id="access<?= $enrolled_course['id'] ?>" onchange="accessUser('<?php echo $enrolled_course['id'] ?>');" value = <?=$enrolled_course['id'] ?>  <?php if ($enrolled_course['access']  == 1): ?> checked <?php endif; ?>>
                                                                    <label  class="custom-control-label"  for="access<?= $enrolled_course['id'];  ?>">Bloquear/Desbloquear Aluno</label>
                                                                </div></td>

                                                            <td>

                                                                <button type="button" class="btn btn-outline-primary btn-icon btn-rounded btn-sm" onclick="alter_date('<?php echo $enrolled_course['id'] ?>');"> <i class="fa fa-save" aria-hidden="true"></i> </button>
                                                                <button type="button" class="btn btn-outline-danger btn-icon btn-rounded btn-sm" onclick="confirm_modal('<?php echo site_url('admin/enrol_history_delete/' . $enrolled_course['id']); ?>');"> <i class="dripicons-trash"></i> </button>
                                                                <button type="button" class="btn btn-outline-info btn-icon btn-rounded btn-sm" onclick="confirm_modal('<?php echo site_url('admin/send_mail_enrol/' . $enrolled_course['id']); ?>');"> <i class="dripicons-mail"></i> </button>
...
<s>
    <script>
        function alter_date(id) {

            var data = '#dateCourse' + id;
            var days_experetion = '#days_experetion' + id;


            $.ajax({
                type: 'POST',
                url: "<?php echo base_url('admin'); ?>/editDadosUser?ajax=true",
                data: {idEnrolCourse: id,
                    days_expiretion: $(days_experetion).val(),
                    valueDataAdded: $(data).val()},
                dataType: 'json',

                success: function (data) {
                    if (data.situacao == true) {
                        alert('Data de inscrição :' + data.mensagem);
...
function accessUser(enrol_id) {

 
  var access;
  if ($('#access'+enrol_id).is(':checked')) {
    access = 1;
  }else{
    access = 0;
  }
  ...
  $.ajax({
    type : 'POST',
  url: "<?php echo base_url('admin'); ?>/bloquearAcessoUserCourse?ajax=true",
  
    data : {id : enrol_id, access : access},
    dataType: 'json',
      success: function (data) {
                    if (data.situacao == true) {
